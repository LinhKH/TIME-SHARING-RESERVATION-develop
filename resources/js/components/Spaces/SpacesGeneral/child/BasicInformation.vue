<template>
    <LayoutForm :title="$t('spaces_general_page.basic_information')" icon="fa fa-info-circle">
        ...
        <LayoutInput :labelText="$t('spaces_general_page.public_status')"
        ...
        <LayoutInput :labelText="$t('spaces_general_page.space_name')" :isRequired="true"
            icon="fa fa-question-circle text-[#337ab7] ml-1">
            <div class="w-[660px] border border-solid border-[#ddd] rounded flex h-8">
                <input type="text" v-model="formData.general_basic_space_name_ja"
        ...
        <LayoutInput :labelText="$t('spaces_general_page.space_name_kana')"
            icon="fa fa-question-circle text-[#337ab7] ml-1">
            <div class="w-[660px] max-w-full h-8 border border-solid border-[#ddd] rounded">
                <input type="text" v-model="formData.general_basic_space_name_kana"
                    class="w-full h-full focus:shadow-lg focus:shadow-cyan-500/50 focus:border-cyan-500/50 px-2 outline-none rounded"
        ...
        <LayoutInput :labelText="$t('spaces_general_page.space_overview')" icon="fa fa-question-circle text-[#337ab7]">
            <div class="w-full border border-solid border-[#ddd] rounded flex h-8">
                <input type="text" v-model="formData.general_basic_space_overview"
                    class="w-full h-full focus:shadow-lg focus:shadow-cyan-500/50 focus:border-cyan-500/50 px-2 outline-none rounded"
                    :placeholder="$t('spaces_general_page.placeholder_3')" />
            </div>
        </LayoutInput>

        <LayoutInput :labelText="$t('spaces_general_page.space_introduction')" :isRequired="true"
            icon="fa fa-question-circle text-[#337ab7] ml-1">
            <div class="w-full border border-solid border-[#ddd] rounded flex h-20">
                <textarea type="text" v-model="formData.general_basic_space_introduction"
                ...
                <div v-for="(item, index) in formData.general_basic_space_purpose_of_uses.length" :key="index">
                    <div class="flex mt-2">
                        ...
                        <select
                            ...
                            <option v-for="(itemSubCategory, indexSubCategory) in formData.general_basic_space_purpose_of_uses[index]
                                .optionSubCategory" :key="indexSubCategory" :value="itemSubCategory.value"
                                :selected="formData.general_basic_space_purpose_of_uses[index].sub_category == itemSubCategory.value">
                                {{ itemSubCategory.label }}
                            </option>
                        </select>
                        <div v-if="isShowBtnAddDelete"
                            ...
                            <i class="fa fa-trash text-[#d43f3a] group-hover:text-white"
                                data-reactid=".e.0.$2.6.0.0"></i>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="flex mt-2">
            <div class="w-64"></div>
            <div class="">
                <button v-if="isShowBtnAddPurpose"

<template>
    <HeaderSpaces :title="$t('spaces_image_page.image_registration')" :textHelp="$t('spaces_image_page.text_help_1')"
        :textLink="$t('spaces_image_page.preview_of_public_page')" :textHelpTop="$t('spaces_image_page.text_desc_1')" />
    <div class="border border-solid border-[#ddd] rounded mt-2">
        <LayoutForm :title="$t('spaces_image_page.image')" icon="fa fa-picture-o mr-1">
            ...
            <draggable v-model="listImages" group="people" @start="dragging = true" @end="dragging = false"
                item-key="order">
                        ...
                        <div class="w-full h-[129px] flex">
                            ...
                            <div class="w-full">
                                <div class="w-full border border-solid border-[#ddd] rounded flex h-8">
                                    <div
                                        class="flex items-center justify-center border-r border-solid border-[#ddd] w-10 bg-[#eee]">
                                        <img :src="JapanIcon" alt="japan" class="w-9 object-cover" />
                                    </div>
                                    <FormKit type="text" v-model="listImages[index].title" outer-class="w-full"
                                        wrapper-class="h-full" inner-class="h-full" validation="required"
                                        :validation-messages="{
                                            required: $t('validation.required'),
                                        }" message-class="text-[red] mt-2 ml-6" help-class=" mt-1"
                                ...
                                <button @click="() => handleDeleteListImages(index, element.id)"
        ...
        <LayoutForm :title="$t('spaces_image_page.degree_photo')" icon="fa fa-picture-o mr-1" :borderFooter="false">
            ...
            <draggable v-model="listImagePanorama" group="people" @start="dragging = true" @end="dragging = false"
                item-key="order">
                <template #item="{ element, index }">
                    <div class="w-full p-5 bg-[#f5f5f5] my-2 border border-solid border-[#e3e3e3] rounded shadow-inner">
                        <div class="w-full h-[129px] flex">
                            ...
                            <div
                                ...
                                <div class="flex">
                                    <div v-if="index" @click="handleClickUp(index, listImagePanorama)"
                                        class="flex items-center justify-center px-2 py-1 border border-solid border-[#e3e3e3] rounded cursor-pointer mr-2">
                                        <i class="fa fa-chevron-up" aria-hidden="true"></i>
                                    </div>
                                    <div v-if="index != listImagePanorama.length - 1"
                            ...
                            <div class="w-full">
                                <div class="w-full border border-solid border-[#ddd] rounded flex h-8">
                                    ...
                                    <FormKit type="text" v-model="listImagePanorama[index].title" outer-class="w-full"
                                        wrapper-class="h-full" validation="required" :validation-messages="{
                                            required: $t('validation.required'),
                                        }" message-class="text-[red] mt-2 ml-6" inner-class="h-full" help-class=" mt-1"
                                        input-class="px-2 w-full h-full focus:shadow-lg outline-none  focus:shadow-lg focus:shadow-cyan-500/50 focus:border-cyan-500/50 " />
                                </div>
                                <button @click="() => handleDeleteListImagePanorama(index, element.id)"
                                    class="py-2 px-3 mt-2 text-[#d43f3a] bg-white border border-solid border-[#d43f3a] rounded hover:bg-[#d43f3a] hover:text-white">
                                    {{ $t("spaces_image_page.delete") }}
                                </button>
                            </div>
            ...
            <input type="file" class="mt-2" multiple accept="image/png, image/jpeg"
                @change="handleChangeImagePanorama" />
        </LayoutForm>
    </div>

<template>
    <div class="font-serif p-2.5">
        ...
        <div class="py-[20px]">
            ...
            <div class="flex mt-[10px]">
                ...
                <div class="flex-[5_5_0%]">
                    <div v-if="detailTour.status === 'change_date_time'" class="mb-[10px]">
                        <p>{{ detailTour.substitute_first_choice_date }} {{ detailTour.substitute_first_choice_time }}
                        </p>
                        <p>{{ detailTour.substitute_second_choice_date }} {{ detailTour.substitute_second_choice_time }}
                        </p>
                        <p>{{ detailTour.substitute_third_choice_date }} {{ detailTour.substitute_third_choice_time }}
                        </p>
                    </div>
                    <div v-if="detailTour.status === 'fix_date_time'" class="mb-[10px]">
                        <div v-if="detailTour.status === 'fix_date_time'">
                            ...
                            "></div>
                            <div v-else>
                                <div>{{ detailTour.fix_choice_date_column_date }} {{
                                    detailTour.fix_choice_date_column_time }}</div>
                            </div>
                        </div>
                    </div>
                    <div>
                        <button v-if="detailTour.status === 'pending_request'"
                        ...
                        <button v-if="detailTour.status !== 'user_cancel'" @click="showModal = true"
            ...
            <div class="flex mt-[10px]">
                ...
                <div class="flex-[5_5_0%] flex justify-start">
                    <ChoiceDateVue :option="detailTour" @changeChoiceDate="handleChangeDate"
      ...
      <p class="mt-[5px]">{{ $t("tour_detail.noti") }}</p> -->
            ...
            <textarea v-model="valueReply" type="textarea"
        ...
        <transition name="pop" appear>
            <div class="modal" role="dialog" v-if="showModal">
                <div>
                    <div v-for="(Alternative, index) in AlternativeList" :key="index">
                        <div class="flex items-center mt-[10px]">
                            <span v-if="Alternative.required"
                        ...
                        <div class="mt-[5px]">
                            <select v-if="Alternative.type == 'select_input'" name="" id=""
                                class="border border-solid border-[#ccc] p-[5px]" v-model="selected">
                                <option v-for="(childAlternative, index) in Alternative.child" :key="index"
                                    :value="childAlternative.title">
                                    {{ childAlternative.title }}
                                </option>
                            </select>
                            <div v-if="Alternative.type == 'calender_input' && showCalender"
                                class="bg-[#bce8f1] p-[10px]">
                                <div>
                                    <div class="mb-[8px]">
                                        <div class="flex items-center">
                                            <span
                                                class="bg-red-400 text-white px-[6px] py-[1px] rounded mr-[4px] text-[10px]">{{
                                        ...
                                        <div class="flex w-full mt-[5px]">
                                            <div class="flex-1 mr-[8px]">
                                                <DatePicker placeholder="2022/01/01"
                                                    v-model="formDate.substitute_first_choice_date" startingView="date"
                                                    class="w-full flex-1 border border-slate-300 focus:outline-none rounded block px-[12px] py-[6px] border border-[#ccc] h-[34px]" />
                                            </div>
                                            <div class="flex-1">
                                                <DatePicker placeholder="09:30"
                                                    v-model="formDate.substitute_first_choice_time" startingView="time"
                                                    inputFormat="HH:mm" minimumView="time"
                                                    class="w-full flex-1 border border-slate-300 focus:outline-none rounded block px-[12px] py-[6px] border border-[#ccc] h-[34px]" />
                                            </div>
                                        </div>
                                    </div>
                                    <div class="mb-[8px]">
                                        ...
                                        <div class="flex w-full mt-[5px]">
                                            <div class="flex-1 mr-[8px]">
                                                <DatePicker placeholder="2022/01/01"
                                                    v-model="formDate.substitute_second_choice_date" startingView="date"
                                                    class="w-full flex-1 border border-slate-300 focus:outline-none rounded block px-[12px] py-[6px] border border-[#ccc] h-[34px]" />
                                            </div>
                                            <div class="flex-1">
                                                <DatePicker placeholder="09:30"
                                                    v-model="formDate.substitute_second_choice_time" startingView="time"
                                                    inputFormat="HH:mm" minimumView="time"
                                                    class="w-full flex-1 border border-slate-300 focus:outline-none rounded block px-[12px] py-[6px] border border-[#ccc] h-[34px]" />
                                            </div>
                                        </div>
                                    ...
                                    <div class="mb-[8px]">
                                        ...
                                        <div class="flex w-full mt-[5px]">
                                            <div class="flex-1 mr-[8px]">
                                                <DatePicker placeholder="2022/01/01"
                                                    v-model="formDate.substitute_third_choice_date" startingView="date"
                                                    class="w-full flex-1 border border-slate-300 focus:outline-none rounded block px-[12px] py-[6px] border border-[#ccc] h-[34px]" />
                                            </div>
                                            <div class="flex-1">
                                                <DatePicker placeholder="09:30"
                                                    v-model="formDate.substitute_third_choice_time" startingView="time"
                                                    inputFormat="HH:mm" minimumView="time"
                                                    class="w-full flex-1 border border-slate-300 focus:outline-none rounded block px-[12px] py-[6px] border border-[#ccc] h-[34px]" />
                                            </div>
                                        </div>
                ...
                <div class="w-[100%] text-center mt-[20px]">
                    <button @click="() => postDataNoneApproval(tourItem)"

<template>
    <div class="p-[5px]">
        ...
        <div>
            <label v-for="(settingItem, index) in listSettingTour" :key="index" :for="settingItem.id"
                class="font-bold mr-[10px] inline-block justify-start items-center"><input class="mr-[3px]"
                    @click="handleChecked(settingItem.id)" :id="settingItem.id" type="checkbox"
                    :checked="settingItem.isChecked" /><span>{{ settingItem.title }}</span></label>
        </div>
        <div class="w-[100%] text-center mt-[20px]">
            <button @click="() => updateSettingTour()"

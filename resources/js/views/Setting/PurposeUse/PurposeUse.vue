<template>
    <div class="">
        <div class="flex flex-wrap">
            <button v-for="(category, indexCategory) in listCategories"
                class="p-2 border border-solid border-[#ddd] rounded block mr-2 mb-2"
                :class="category.active ? 'bg-[#337ab7] text-white' : 'text-[#337ab7] bg-white'" :key="indexCategory"
        ...
        <table class="table w-full border border-solid border-[#ccc] border-collapse" cellspacing="1">
            <thead>
                <tr>
                    <th class="border border-solid border-[#ccc] border-collapse p-[8px] text-start"
                        v-for="(newHeaderItem, index) in purposeUseHeader" :class="newHeaderItem.width" scope="col"
                        :key="index">
                        {{ newHeaderItem.name }}
                    </th>
                </tr>
            ...
            <tbody>
                <tr v-for="(purposeItem, index) in listPurposeUse" :key="index" class="odd:bg-[#f9f9f9] group">
                    <td class="border border-solid border-[#ccc] border-collapse p-2.5 text-start" scope="row">
                        <img :src="purposeItem.url_img" @click="() => handleDetailPurpose(purposeItem.id)"
                            class="text-[#337ab7] cursor-pointer hover:underline w-full object-cover" />
                    </td>
                    <td class="border border-solid border-[#ccc] border-collapse p-2.5 text-start" scope="row">
                        <span @click="() => handleDetailPurpose(purposeItem.id)"
                            class="text-[#337ab7] cursor-pointer hover:underline">{{ purposeItem.title_ja ||
                                purposeItem.title_en || purposeItem.title_jp }}</span>
                    </td>
    ...
    <Pagination v-if="pagination" :pageCurrent="pagination.current_page"
        :totalPage="Math.ceil(pagination.total / pagination.per_page)" @onBack="handleBackPage"

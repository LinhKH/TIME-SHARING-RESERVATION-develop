    ...
    <div class="flex flex-wrap" :style="gap ? 'gap:' + gap : ''">
        ...
        <div class="flex-1">
            <label class="flex flex-row item-center h-[34px]" :style="'width: ' + width">
                <DatePicker v-model="time" startingView="time" inputFormat="HH:mm" minimumView="time"
                ...
                <span v-if="tag" v-html="tag"
                    class="flex items-center p-3 bg-[#eee] rounded-tr-[4px] rounded-br-[4px] border border-[#ccc] border-l-0 text-[#555] text-[14px] text-[400]"></span>
            </label>
        </div>
    </div>
</template>

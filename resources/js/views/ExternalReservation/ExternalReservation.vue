<template>
    <div class="p-3">
        <ul
            class="flex justify-content-start align-items-center border-b border-solid border-[#ddd] pt-2.5 pl-2.5 mb-5">
            <li class="siderbar__item" :class="[currentStep == siderbar.id ? 'active' : '']"
                v-for="(siderbar, index) in siderBarSteps" :key="index">
                <a :class="[currentStep == siderbar.id ? classActive : '']"
                    class="block py-2.5 px-4 cursor-pointer bg-white text-[#337ab7]" :href="`${siderbar.url}`">{{
                    siderbar.title }}</a>
            </li>
        </ul>
        ...
        <div v-if="isShowFilter" class="p-3 mt-2">
            <div class="mb-[10px] flex mt-2 items-center">
                ...
                <div class="flex-1">
                    <Select :optionsSelect="optionsSelectExemple" :inputKey="'input_1'" :currentValue="'item2'"
                        @changeValue="updateValueInput" />
                </div>
            </div>

            <BaseFormSearchExternal :label="$t('external_reservation.status')" :isExtend="true">
                <MultipleCheckbox :optionsCheckbox="optionsCheckBoxStatus" :inputKey="'input_3'"
                    :currentValue="currentOptionsCheckBoxStatus" @changeValue="updateValueCheckBox1" />
            </BaseFormSearchExternal>

            <BaseFormSearchExternal :label="$t('external_reservation.purpose_use')" :isExtend="true">
                <MultipleCheckbox :optionsCheckbox="optionsCheckBoxPurposeUse" :inputKey="'input_3'"
                    :currentValue="currentOptionsCheckBoxPurposeUse" @changeValue="updateValueCheckBox1" />
            </BaseFormSearchExternal>
            ...
            <BaseFormSearchExternal :label="$t('external_reservation.oder_type')" :isExtend="true">
                <MultipleCheckbox :optionsCheckbox="optionsCheckBoxOderType" :inputKey="'input_5'"
                    :currentValue="currentOptionsCheckBoxOderType" @changeValue="updateValueCheckBox1" />
            </BaseFormSearchExternal>

            <BaseFormSearchExternal v-for="item in dataForm" :key="item.id" :label="item.label"
                :options="item.radioOption" :type="item.type" />
            <BaseFormSearchExternal :label="$t('external_reservation.people')" :iconClass="'fa fa-user-plus'"
                :isExtend="true">
                <div class="flex items-center">
                    <div class="flex">
                        <input
                            ...
                            type="text" name="endPeople" id="startPeople" />
                        <label class="w-1/4 p-1 bg-slate-200 border border-solid border-[#ddd] rounded"
                            for="startPeople">{{
                    ...
                    <div class="flex">
                        <input
                            ...
                            type="text" name="endPeople" id="endPeople" />
                        <label class="w-1/4 p-1 bg-slate-200 border border-solid border-[#ddd] rounded"
                            for="endPeople">{{
            ...
            <BaseFormSearchExternal :label="$t('external_reservation.schedule_use_day')" :isExtend="true">
                <div class="flex items-center">
                    <div class="flex h-8 border rounded mr-4">
                        ...
                        <div
                            class="h-full px-2 bg-[#eee] border-r border-[#dbdbdb] text-sm flex items-center justify-center">
                            {{ $t("list_customer_dropdown.start_date") }}
                        </div>
                        <div class="flex items-center justify-center">
                            <datepicker class="px-2 w-40 outline-none text-center text-sm"
                                v-model="startDatePickerUse" />
                        </div>
                    </div>

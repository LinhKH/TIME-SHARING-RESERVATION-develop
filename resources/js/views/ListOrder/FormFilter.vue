<template>
	<LayoutInput :iconPrev="itemForm.iconPrev" :labelText="itemForm.label" v-for="(itemForm, index) in listForm"
		:key="index" :isRequired="itemForm.isRequired" :customCss="`mb-4`">
		<template v-if="itemForm.type === TYPE_COMMON.twoSelect">
			<div class="flex">
				<Select class="mr-4" :inputKey="`main_category`" :optionsSelect="itemForm.optionLeft"
					@changeValue="(data) => (formData[data.key] = data.value)"></Select>
				<Select :inputKey="`sub_category`" :optionsSelect="itemForm.optionRight"
		...
		<template v-if="itemForm.type === TYPE_COMMON.multipleSelect">
			<Select class="mr-4" :inputKey="`test_key_1`" :optionsSelect="itemForm.option"
				@changeValue="(data) => (formData[data.key] = data.value)" :multiple="true" :size="10"></Select>
		</template>

		<template v-if="itemForm.type === TYPE_COMMON.inputTextSelect">
			<div class="min-w-fit w-full relative">
				<Select class="mr-4" :inputKey="`main_category`" :optionsSelect="itemForm.option"
					@changeValue="(data) => (formData[data.key] = data.value)"></Select>
			</div>
		</template>

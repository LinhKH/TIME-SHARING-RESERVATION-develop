...
abstract class AbstractBaseRepository implements RepositoryInterface
{
     ...
     * @return Collection
     */
    public function findManyByValues($key, array $values)
    {
        return $this->model
            ->whereIn($key, $values)
            ->orderBy($this->orderBy, $this->sortMethod)
    ...
    public function paginateByCondition(array $condition, $perPage = 20)
    {
        return $this->model
            ->where($condition)
            ->orderBy($this->orderBy, $this->sortMethod)
    ...
    public function paginatePageCurrent($items, $perPage, $page = null, $options = [])
    {
     ...
     * @throws RepositoryException
     */
    public function updateOneByCondition(array $condition, array $data = []): bool
    {
        if (!is_array($data) || empty($data)) {
            throw new RepositoryException;
        }
     ...
     * @return boolean
     */
    public function deleteOneByKey($key, $value): bool
    {
        return $this->model
            ->where($key, '=', $value)
            ->delete();
     ...
     * @return boolean
     */
    public function deleteManyBy($key, $value): bool
    {
        return $this->model
            ->where($key, $value)
            ->take($this->limit)
    ...
    public function explodeStringToArray($input): array
    {
    ...
    function executeTransaction($func): bool
    {
        DB::beginTransaction();
        try {
            $func();
